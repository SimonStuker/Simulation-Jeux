...
{
    let matching_cond: [bool; N_TABLE_CARDS] = std::array::from_fn(|i| fx_cond(table_cards[i].0) );
...
pub fn fn_three_consecutive_ordered(table_cards: &TableCards) -> bool {
    let values: [i8; N_TABLE_CARDS] = std::array::from_fn(|i| table_cards[i].0.value.get());
    values.windows(3)
        .any(|w| matches!(
            w,
...
pub fn fn_col_sum(table_cards: &TableCards, col: CardColor) -> i8 {
    table_cards.iter().filter(|c| c.0.color == col).map(|c| c.0.value.get()).sum()
}
