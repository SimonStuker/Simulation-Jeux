    ...
    F: Fn(&'static Card) -> u8
{
    let all_mask: u8 = table_cards.iter()
        .map(|card| fx_to_bitmask(card.card()))
...
pub fn fn_three_consecutive_ordered(table_cards: &TableCards) -> bool {
    let values: [i8; N_TABLE_CARDS] = std::array::from_fn(|i| table_cards[i].card().value.get());
    values.windows(3)
        .any(|w| matches!(
            w,
...
pub fn fn_col_sum(table_cards: &TableCards, col: CardColor) -> i8 {
    table_cards.iter().filter(|c| c.card().color == col).map(|c| c.card().value.get()).sum()
}
